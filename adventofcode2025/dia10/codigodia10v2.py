...
def generateListsOfPossibleButtonPresses(i,listOfPressedButtons,JoltageThatLimitsMaxTimesButtonsCanBePressed,WhichButtonsActivateEachJoltage):
    ...
    if i == len(buttons):
        ...
        for n in range(len(listOfPressedButtons)):
            for item in buttons[n]:
                joltagesForThisList[item]+=1 * listOfPressedButtons[n]
                
        if joltagesForThisList == targetJoltages:
            raise SolutionFound(sum(listOfPressedButtons))
        return
    ...
    for n in range(0,buttonPressesLeft+1):
        ...
        generateListsOfPossibleButtonPresses(i+1,new_list,JoltageThatLimitsMaxTimesButtonsCanBePressed,WhichButtonsActivateEachJoltage)
...
for element in orderedElectricStuff:
    ...
    buttons = sorted(element[0], key=len)
    ...
    try:
        ...
        listOfButtonPresses.append(e.min_presses)
